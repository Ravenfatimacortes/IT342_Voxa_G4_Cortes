...
const EditSurvey = () => {
  ...
  const fetchSurvey = async () => {
    try {
      ...
      if (survey.status === 'PUBLISHED') {
        toast.error('Cannot edit published survey');
  ...
  const updateOption = (questionIndex, optionIndex, value) => {
    setValue(`questions.${questionIndex}.options.${optionIndex}`, value);
  };
  ...
  const changeQuestionType = (questionIndex, newType) => {
    ...
    if (newType === 'MULTIPLE_CHOICE') {
      ...
      if (currentOptions.length === 0) {
        setValue(`questions.${questionIndex}.options`, ['Option 1', 'Option 2']);
      }
    } else {
  ...
  const saveChanges = async (data) => {
    try {
      ...
      toast.error(error.response?.data?.error || 'Failed to update survey');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const publishSurvey = async (data) => {
    try {
      setPublishing(true);
      await api.put(`/admin/surveys/${id}`, data);
      await api.post(`/admin/surveys/${id}/publish`);
